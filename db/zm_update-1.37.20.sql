...
VALUES(227, 'ZM_OPT_ALS_TRIGGEREVENT', '0', 'boolean', 'no', 'yes|no', '(?^i:^([yn]))', ' ($1 =~ /^y/) ? ''yes'' : ''no'' ', '
      Trigger an event on Human Detected alarm from ALARM SERVER to ZoneMinder. Requires the zmTrigger option Enabled', 
      ' ', 'system', 0, 0, 0, NULL);


INSERT INTO zm.Config
(Id, Name, Value, `Type`, DefaultValue, Hint, Pattern, Format, Prompt, Help, Category, Readonly, Private, `System`, Requires)
VALUES(228, 'ZM_OPT_ALS_PORT', '15002', 'integer', '15002', 'XXXXX', '(?^i:^([yn]))', ' ($1 =~ /^y/) ? ''yes'' : ''no'' ', 'Port Number to receive alarms from Alarm Server', 
     '','system', 0, 0, 0, NULL);
