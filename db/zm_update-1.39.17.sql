--
-- AI object-detection data model.
--
-- Adds the per-monitor object-detection columns and the AI_* tables used by
-- the object-detection management UI, then seeds the COCO 2017 dataset.
--
-- All statements are idempotent so re-running (or running after a fresh
-- install that already has these objects) is a no-op.
--
...
-- AnalysisImageOpacity: alpha applied when compositing the analysis/annotated
-- image over the source frame (0-255).
SET @s = (SELECT IF(
    (SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS WHERE table_schema = DATABASE()
     AND table_name = 'Monitors' AND column_name = 'AnalysisImageOpacity') > 0,
...
-- ObjectDetection: name of the detection backend for this monitor, or 'none'.
-- VARCHAR(16) so backends can be added without a schema change.
SET @s = (SELECT IF(
    (SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS WHERE table_schema = DATABASE()
     AND table_name = 'Monitors' AND column_name = 'ObjectDetection') > 0,
"SELECT 'Column ObjectDetection already exists in Monitors'",
"ALTER TABLE `Monitors` ADD COLUMN `ObjectDetection` VARCHAR(16) NOT NULL DEFAULT 'none' AFTER `AnalysisImageOpacity`"
));
PREPARE stmt FROM @s; EXECUTE stmt; DEALLOCATE PREPARE stmt;

-- ObjectDetectionModel: path/name of the model file used by the backend.
SET @s = (SELECT IF(
    (SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS WHERE table_schema = DATABASE()
     AND table_name = 'Monitors' AND column_name = 'ObjectDetectionModel') > 0,
"SELECT 'Column ObjectDetectionModel already exists in Monitors'",
"ALTER TABLE `Monitors` ADD COLUMN `ObjectDetectionModel` VARCHAR(255) NOT NULL DEFAULT '' AFTER `ObjectDetection`"
));
PREPARE stmt FROM @s; EXECUTE stmt; DEALLOCATE PREPARE stmt;

-- ObjectDetectionObjectThreshold: minimum confidence for a detection to count.
SET @s = (SELECT IF(
    (SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS WHERE table_schema = DATABASE()
     AND table_name = 'Monitors' AND column_name = 'ObjectDetectionObjectThreshold') > 0,
"SELECT 'Column ObjectDetectionObjectThreshold already exists in Monitors'",
"ALTER TABLE `Monitors` ADD COLUMN `ObjectDetectionObjectThreshold` FLOAT NOT NULL DEFAULT 0.4 AFTER `ObjectDetectionModel`"
));
PREPARE stmt FROM @s; EXECUTE stmt; DEALLOCATE PREPARE stmt;

-- ObjectDetectionNMSThreshold: non-maximum-suppression IoU threshold.
SET @s = (SELECT IF(
    (SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS WHERE table_schema = DATABASE()
     AND table_name = 'Monitors' AND column_name = 'ObjectDetectionNMSThreshold') > 0,
"SELECT 'Column ObjectDetectionNMSThreshold already exists in Monitors'",
"ALTER TABLE `Monitors` ADD COLUMN `ObjectDetectionNMSThreshold` FLOAT NOT NULL DEFAULT 0.25 AFTER `ObjectDetectionObjectThreshold`"
));
PREPARE stmt FROM @s; EXECUTE stmt; DEALLOCATE PREPARE stmt;

-- Widen ObjectDetection to VARCHAR(16) in case an older upgrade path left it as
-- an ENUM or a narrower VARCHAR.
...
--
-- Seed the COCO 2017 dataset (80 classes) and default detection settings.
--
