...
if ( ! ($check || $freshen || $rename || $zoneFix || $migrateEvents || $version) ) {
    if ( $Config{ZM_DYN_DB_VERSION} ) {
        ...
        print( STDERR "No DB Version. Please give a valid option\n" );
...
if ( $version )
{
    ...
    if ( $interactive )
    {
        ...
        if ( $response =~ /^[yY]$/ )
        {
            ...
            my $backup = "/var/tmp/zm/".$Config{ZM_DB_NAME}."-".$version.".dump";
	...
	if ( $cascade || $version lt "1.27.0" )
	{
		...
		my $columns = $sth->{'NAME'};
		if ( ! grep( /^ServerHost$/, @$columns ) ) {
			$dbh->do(q{alter table Monitors add column `ServerHost` varchar(64) not null default '' after `Name`;} );
		} # end if
		if ( ! grep(/^JPGPath$/, @$columns ) ) {
			$dbh->do(q{alter table Monitors add column `JPGPath` VARCHAR(255) NOT NULL default '' after `Path`;});
			$dbh->do(q{Update Monitors set `JPGPath`=`Path`;});
		} # end if
		if ( ! grep(/^MJPGPath$/, @$columns ) ) {
			$dbh->do(q{alter table Monitors add column `MJPGPath` VARCHAR(255) NOT NULL default '' after `JPGPath`;});
			$dbh->do(q{Update Monitors set `MJPGPath`=`Path`;});
		} # end if
		$sth->finish();

		$cascade = !undef if ! $dbh->errstr;
	}
	if ( $cascade )
	{
