# ==========================================================================
#
# ZoneMinder ONVIF SOAP IP Control Protocol Module, $Date: 2009-11-25 09:20:00 +0000 (Wed, 04 Nov 2009) $, $Revision: 0001 $
# Copyright (C) 2001-2008 Philip Coombes
# This is adapted from the Netcat module by Andrew Bauer and has been Modified for SOAP ONVIF IP Cameras by Michael Berna 
#
...
# ==========================================================================
#
# This module contains the first implementation of the IP camera control
# protocol
#
package ZoneMinder::Control::onvifSoap;
...
#   eg : 10.1.2.1:80
#        10.0.100.1:80
#        username:password@10.0.100.1:80
#
#
...
sub open {
  ...
  if ($profileToken eq '') { $profileToken = 'PROFILE_000'; }
...
sub randomChar()
{
    my $randNum = int(rand(61));
    
    if($randNum <= 9)
    {
        return $randNum;
    }
    elsif($randNum >= 10 && $randNum < 36)
    {
        return chr($randNum+55);
    }
    else
    {
        return chr($randNum+61);
    }
}
...
sub sendCmd 
{
  ...
  #my $server_endpoint = 'http://'.$address.':'.$port.'/onvif/ptz_service';
...
sub getCamParams 
{
  ...
  my $msg = '<s:Envelope xmlns:s="http://www.w3.org/2003/05/soap-envelope"><s:Body xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xsd="http://www.w3.org/2001/XMLSchema"><GetImagingSettings xmlns="http://www.onvif.org/ver20/imaging/wsdl"><VideoSourceToken>PROFILE_000</VideoSourceToken></GetImagingSettings></s:Body></s:Envelope>';
