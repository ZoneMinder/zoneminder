...
static int ni_yolov5_get_boxes(YoloModelCtx *ctx, uint32_t img_width,
        uint32_t img_height, struct roi_box **roi_box, int *roi_num)
{
    ...
    *roi_num = 0;
    ...
    ret = ni_get_yolov5_detections(ctx, sequence, normalize_box);
    if (ret < 0) {
...
static int create_yolov5_model(YoloModelCtx *ctx, ni_network_data_t *network_data,
...
static void destroy_yolov5_model(YoloModelCtx *ctx)
{
    if (ctx->out_tensor) {
        int i;
...
YoloModel yolov5 = {
    .create_model  = create_yolov5_model,
    .destroy_model = destroy_yolov5_model,
