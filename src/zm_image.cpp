...
void Image::AssignDirect(
  ...
  if (av_size < 0 || av_linesize < 0) {
    Error("AssignDirect: av_image sizing failed for %s %ux%u",
...
void Image::Assign(const Image &image) {
  ...
  if ( !buffer
    ...
    if ( holdbuffer && buffer ) {
      if ( new_size > allocation ) {
        Error("Held buffer is undersized for assigned buffer (need %u, have %lu)",
              new_size, allocation);
        return;
      }
    } else {
...
#define EXIF_CODE       0xE1
  ...
  if (imagePixFormat == AV_PIX_FMT_YUV420P || imagePixFormat == AV_PIX_FMT_YUVJ420P
   || imagePixFormat == AV_PIX_FMT_YUV422P || imagePixFormat == AV_PIX_FMT_YUVJ422P) {
    ...
    if (av_image_fill_arrays(plane, plane_linesizes, buffer,
                             imagePixFormat, width, height, 32) < 0) {
      Error("WriteJpeg: av_image_fill_arrays failed for %s",
            av_get_pix_fmt_name(imagePixFormat));
      jpeg_abort_compress(cinfo);
      fl.l_type = F_UNLCK;
      fcntl(raw_fd, F_SETLK, &fl);
...
bool Image::EncodeJpeg(JOCTET *outbuffer, size_t *outbuffer_size, int quality_override) const {
  ...
  if (is_yuv_planar) {
    ...
    if (av_image_fill_arrays(plane, plane_linesizes, buffer,
                             imagePixFormat, width, height, 32) < 0) {
      Error("EncodeJpeg: av_image_fill_arrays failed for %s",
            av_get_pix_fmt_name(imagePixFormat));
      jpeg_abort_compress(cinfo);
      return false;
    }
...
void Image::Scale(const unsigned int new_width, const unsigned int new_height) {
  ...
  if (new_size < 0) {
    Error("Scale: av_image_get_buffer_size failed for %s %ux%u",
          av_get_pix_fmt_name(format), new_width, new_height);
    return;
  }
  ...
  SWScale swscale;
  swscale.init();
  if (swscale.Convert(buffer, allocation,
                      scale_buffer, scale_buffer_size,
                      format, format,
                      width, height,
  ...
  AssignDirect(new_width, new_height, colours, subpixelorder,
               scale_buffer, scale_buffer_size, ZM_BUFTYPE_ZM);
}
