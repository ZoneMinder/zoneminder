...
std::string load_monitor_sql =
    ...
    "`Janus_Use_RTSP_Restream`, `Janus_RTSP_User`, "
    ...
    "`SectionLength`, `SectionLengthWarn`, `MinSectionLength`, "
    "`EventCloseMode`+0, "
    ...
    "`ONVIF_URL`, `ONVIF_EVENTS_PATH`, `ONVIF_Username`, `ONVIF_Password`, "
    "`ONVIF_Options`, "
...
/*
   ...
   "SELECT `Id`, `Name`, `Deleted`, `ServerId`, `StorageId`, `Type`,
   `Capturing`+0, `Analysing`+0, `AnalysisSource`+0, `AnalysisImage`+0,"
   "`Recording`+0, `RecordingSource`+0, `Decoding`+0, RTSP2WebEnabled,
   RTSP2WebType, `RTSP2WebStream`+0," "JanusEnabled, JanusAudioEnabled,
   Janus_Profile_Override, Janus_Use_RTSP_Restream, Janus_RTSP_User,
   Janus_RTSP_Session_Timeout, " "LinkedMonitors, `EventStartCommand`,
   `EventEndCommand`, " "AnalysisFPSLimit, AnalysisUpdateDelay, MaxFPS,
   AlarmMaxFPS," "Device, Channel, Format, V4LMultiBuffer, V4LCapturesPerFrame,
   " // V4L Settings "Protocol, Method, Options, User, Pass, Host, Port, Path,
   SecondPath, Width, Height, Colours, Palette, Orientation+0, Deinterlacing,
   RTSPDescribe, " "SaveJPEGs, VideoWriter, EncoderParameters," "OutputCodec,
   Encoder, OutputContainer, RecordAudio, WallClockTimestamps," "Brightness,
   Contrast, Hue, Colour, " "EventPrefix, LabelFormat, LabelX, LabelY,
   LabelSize," "ImageBufferCount, `MaxImageBufferCount`, WarmupCount,
...
bool Monitor::Analyse() {
  ...
  Debug(3,
        "Motion detection is enabled?(%d) signal(%d) signal_change(%d) trigger "
        "state(%s) image index %d",
        shared_data->analysing, signal, signal_change,
        TriggerState_Strings[trigger_data->trigger_state].c_str(),
        snap->image_index);
