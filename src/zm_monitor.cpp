...
std::string load_monitor_sql =
    ...
    "`Go2RTCEnabled`, `Go2RTCType`, `Go2RTCStream`+0, "
...
void Monitor::Load(MYSQL_ROW dbrow, bool load_zones = true, Purpose p = QUERY)
{
  ...
  col++;
  if (onvif_alarm_txt.empty())
    onvif_alarm_txt = "MotionAlarm";
...
bool Monitor::connect()
{
  ...
  mem_size = sizeof(SharedData) + sizeof(TriggerData) + (zone_count * sizeof(int))                                                   // Per zone scores
             + sizeof(VideoStoreData)                                                                                                // Information to pass back to the capture process
             + (image_buffer_count * sizeof(struct timeval)) + (image_buffer_count * image_size) + (image_buffer_count * image_size) // alarm_images
  ...
  if (purpose == CAPTURE)
  {
    ...
    { //
      ...
      if (onvif_options.find("closes_event") != std::string::npos)
...
#if MOSQUITTOPP_FOUND
      if (mqtt)
        mqtt->send(stringtf("Capturing at %.2lf fps, capturing bandwidth %ubytes/sec Analysing at %.2lf fps",
...

  {
    ...
    if (trigger_data->trigger_state != TriggerState::TRIGGER_OFF)
    {
      ...
      if (signal)
      {
        ...
        {
          if (
              (shared_data->recording == RECORDING_ALWAYS) or
...
bool Monitor::Decode()
{
  ...
  if ((!packet->image) and packet->packet->size and !packet->in_frame)
  {
    if ((decoding == DECODING_ALWAYS) or
        ((decoding == DECODING_ONDEMAND) and (this->hasViewers() or (shared_data->last_write_index == image_buffer_count))) or
  ...
  if ((analysis_image == ANALYSISIMAGE_YCHANNEL) && packet->in_frame && (((AVPixelFormat)packet->in_frame->format == AV_PIX_FMT_YUV420P) || ((AVPixelFormat)packet->in_frame->format == AV_PIX_FMT_YUVJ420P)))
  {
...
#if MOSQUITTOPP_FOUND
  if (mqtt)
    mqtt->send(stringtf("event start: %" PRId64, event->Id()));
#endif
...
#if MOSQUITTOPP_FOUND
  if (mqtt)
    mqtt->send(stringtf("event end: %" PRId64, event->Id()));
#endif
...
int Monitor::PrimeCapture()
{
  ...
  {
    if (!dest_frame)
      dest_frame = av_frame_ptr{zm_av_frame_alloc()};
    if (!decoder_it)
      decoder_it = packetqueue.get_video_it(false);
    if (!decoder)
    {
...
void Monitor::get_ref_image()
{
  ...
  while (
      (
          !(snap_lock = packetqueue.get_packet(analysis_it)) or
