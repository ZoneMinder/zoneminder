...
class Monitor : public std::enable_shared_from_this<Monitor> {
 ...
 protected:
  ...
  unsigned int storage_id;  // Id of the Storage Object, which currently will
                            // just provide a path, but in future may do more.
  CameraType type;
  ...
  int event_count;
  int last_capture_image_count;  // last value of image_count when calculating
                                 // capture fps
  int analysis_image_count;  // How many frames have been processed by analysis
                             // thread.
  int decoding_image_count;  // How many frames have been processed by analysis
                             // thread.
  int motion_frame_count;    // How many frames have had motion detection
                             // performed on them.
  int last_motion_frame_count;  // last value of motion_frame_count when
                                // calculating fps
  int ready_count;
  ...
  av_frame_ptr
      dest_frame;  // Used by decoding thread doing colorspace conversions
  ...
  Image delta_image;
  Image ref_image;
  Image alarm_image;  // Used in creating analysis images, will be initialized
                      // in Analysis
 ...
 public:
  ...
  inline int ShmValid() const {
    if (shared_data && shared_data->valid) {
      ...
      Debug(3,
            "Shared data is valid, checking heartbeat %" PRIi64 " - %" PRIi64
            " = %" PRIi64 "  < %f",
  ...
  inline bool Ready() const {
    ...
    if (decoding_image_count > ready_count) {
      Debug(4, "Ready because decoding_image_count(%d) > ready_count(%d)",
            decoding_image_count, ready_count);
      return true;
    }
    Debug(4, "Not ready because decoding_image_count(%d) <= ready_count(%d)",
          decoding_image_count, ready_count);
    return false;
  }
  ...
  */
  ...
  int GetAudioFrequency() const {
    return shared_data ? shared_data->audio_frequency : -1;
  }
  int GetAudioChannels() const {
    return shared_data ? shared_data->audio_channels : -1;
  }
  ...
  AVStream *GetAudioStream() const {
    return camera ? camera->getAudioStream() : nullptr;
  };
  AVCodecContext *GetAudioCodecContext() const {
    return camera ? camera->getAudioCodecContext() : nullptr;
  };
  AVStream *GetVideoStream() const {
    return camera ? camera->getVideoStream() : nullptr;
  };
  ...
  // unsigned int DetectBlack( const Image &comp_image, Event::StringSet
  // &zoneSet );
  ...
  Event *openEvent(const std::shared_ptr<ZMPacket> &snap,
