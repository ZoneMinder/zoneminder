...
Monitor::Go2RTCManager::Go2RTCManager(Monitor *parent_) : parent(parent_),
                                                          Go2RTC_Healthy(false)
{
    ...
    if ((config.rtsp2web_path != nullptr) && (config.rtsp2web_path[0] != '\0'))
    {
        ...
        if (Go2RTC_endpoint.back() == '/')
            Go2RTC_endpoint.pop_back();
    }
    else
    {
...
Monitor::Go2RTCManager::~Go2RTCManager()
{
    remove_from_Go2RTC();
}
...
int Monitor::Go2RTCManager::add_to_Go2RTC()
{
    ...
    std::string endpoint = Go2RTC_endpoint + "/stream/" + std::to_string(parent->id) + "/add";
    ...
    if (!rtsp_second_path.empty())
    {
        postData += ", \"1\" : {"
                    ...
                    rtsp_second_path + "\", \"on_demand\": true, \"debug\": false, \"status\": 0}";
    }
    postData += "}"
                "}";
...
int Monitor::Go2RTCManager::remove_from_Go2RTC()
{
    ...
    std::string endpoint = Go2RTC_endpoint + "/stream/" + std::to_string(parent->id) + "/delete";
    std::string response;
    ...
    {
        Debug(1, "Removed stream from Go2RTC: %s", remove_newlines(response).c_str());
    }
