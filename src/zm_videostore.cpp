...
void VideoStore::flush_codecs() {
  ...
  if (video_out_ctx->codec && ( video_out_ctx->codec->capabilities & 
    ...
    while (0 > zm_send_frame_receive_packet(video_out_ctx, nullptr, pkt.get())) {
  ...
  if (audio_out_codec) {
    ...
    while (zm_resample_get_delay(resample_ctx, audio_out_ctx->sample_rate)) {
      ...
      if (zm_add_samples_to_fifo(fifo, out_frame)) {
        ...
        if (zm_get_samples_from_fifo(fifo, out_frame)) {
          if (zm_send_frame_receive_packet(audio_out_ctx, out_frame, pkt.get()) > 0) {
            av_packet_guard pkt_guard{pkt};
    ...
    while (av_audio_fifo_size(fifo) > 0) {
      ...
      if (av_audio_fifo_read(fifo, (void **)out_frame->data, frame_size)) {
        if (zm_send_frame_receive_packet(audio_out_ctx, out_frame, pkt.get())) {
          av_packet_guard pkt_guard{pkt};
...
int VideoStore::writeVideoFramePacket(const std::shared_ptr<ZMPacket> zm_packet) {
  ...
  if (monitor->GetOptVideoWriter() == Monitor::ENCODE) {
    ...
    if (video_first_pts == AV_NOPTS_VALUE) {
      ...
      Debug(2, 
            "Setting pts for frame(%d) to (%" PRId64 ") from (start %" PRIu64 " - %" PRIu64 " - secs(%" PRIi64 ".%" PRIi64 ") @ %d/%d",
            frame_count,
            frame->pts,
            video_first_pts,
            in_pts,
    ...
    int ret = zm_send_frame_receive_packet(video_out_ctx, frame, opkt.get());
    if (ret <= 0) {
      if (ret < 0) {
    ...
    Debug(2, "Timebase conversions using %d/%d -> %d/%d",
        video_out_ctx->time_base.num,
        video_out_ctx->time_base.den,
    ...
    if (zm_packet->in_frame) {
      ...
      if (
        ...
        Warning( "duration from ipkt: = duration(%" PRId64 ") => (%" PRId64 ") (%d/%d) (%d/%d)",
#if LIBAVCODEC_VERSION_CHECK(60, 3, 0, 3, 0)
            zm_packet->in_frame->duration,
#else
            zm_packet->in_frame->pkt_duration,
        ...
        duration = av_rescale_q(
              ...
              video_in_stream->time_base,
              video_out_stream->time_base);
        Warning( "duration calc: pts(%" PRId64 ") - last_pts(%" PRId64 ") = (%" PRId64 ") => (%" PRId64 ")",
...
#if LIBAVCODEC_VERSION_CHECK(57, 64, 0, 64, 0)
      Debug(2, "duration calc: pts %" PRId64 " - last_pts %" PRId64 " = %" PRId64 ,
          opkt->pts,
          video_last_pts,
          opkt->duration
          );
    } else {
      Debug(2, "Setting duration to %" PRId64 " dts is %" PRId64 " pts is %" PRId64, opkt->duration, opkt->dts, opkt->pts);
#endif
    }

    video_last_pts = opkt->pts;
    Debug(2, "Setting video_last_pts to %" PRId64 " because it's the first packet", video_last_pts);
  } else { // Passthrough
...
int VideoStore::writeAudioFramePacket(const std::shared_ptr<ZMPacket> zm_packet) {
  ...
  if (audio_first_dts == AV_NOPTS_VALUE) {
    ...
    Debug(1, "audio first_dts to %" PRId64, audio_first_dts);
  }
  ...
  if (audio_out_codec) {
    ...
    while (zm_resample_audio(resample_ctx, input_frame, out_frame)) {
      ...
      if (zm_send_frame_receive_packet(audio_out_ctx, out_frame, opkt.get()) <= 0)
        break;
...
/*    Debug(1,
  ...
  Debug(2, "next_dts for stream %d has become %" PRId64 " last_dts %" PRId64,
