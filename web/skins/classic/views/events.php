...
?>
            <tr<?php if ($event->Archived()) echo ' class="archived"' ?>>
              ...
              <td class="colCause">
                <?php echo makePopupLink( '?view=eventdetail&amp;eid='.$event->Id(), 'zmEventDetail', 'eventdetail', validHtmlStr($event->Cause()), canEdit( 'Events' ), 'title="'.htmlspecialchars($event->Notes()).'"' ) ?>
                <?php if ($event->Notes()) echo '<br/><div class="small text-nowrap text-muted">'.$event->Notes().'</div>' ?>
              </td>
